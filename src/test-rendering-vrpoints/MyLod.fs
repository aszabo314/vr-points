...
type LodRenderer(ctx : Context, manager : ResourceManager, state : PreparedPipelineState, config : LodRendererConfig, roots : aset<LodTreeInstance>)  =
    ...
    let getRootUniform (name : string) (root : ILodTreeNode) : Option<IAdaptiveValue> =
        ...
        rootCache.GetOrAdd(name, fun name ->
            ...
            | "LeftModelViewTrafos"          -> (getRootTrafo root, config.views) ||> AVal.map2 (fun a b -> a * b.[0]) :> IAdaptiveValue |> Some
            | "LeftModelViewTrafosInv"       -> (getRootTrafo root, config.views) ||> AVal.map2 (fun a b -> a * b.[0]) |> AVal.map (fun t -> t.Inverse) :> IAdaptiveValue |> Some
            | "LeftModelViewProjTrafos"      -> (getRootTrafo root, config.views, config.projs) |||> AVal.map3 (fun a b c -> a * b.[0] * c.[0]) :> IAdaptiveValue |> Some
            | "LeftModelViewProjTrafosInv"   -> (getRootTrafo root, config.views, config.projs) |||> AVal.map3 (fun a b c -> a * b.[0] * c.[0]) |> AVal.map (fun t -> t.Inverse) :> IAdaptiveValue |> Some
            
            | "RightModelViewTrafos"          -> (getRootTrafo root, config.views) ||> AVal.map2 (fun a b -> a * b.[1]) :> IAdaptiveValue |> Some
            | "RightModelViewTrafosInv"       -> (getRootTrafo root, config.views) ||> AVal.map2 (fun a b -> a * b.[1]) |> AVal.map (fun t -> t.Inverse) :> IAdaptiveValue |> Some
            | "RightModelViewProjTrafos"      -> (getRootTrafo root, config.views, config.projs) |||> AVal.map3 (fun a b c -> a * b.[1] * c.[1]) :> IAdaptiveValue |> Some
            | "RightModelViewProjTrafosInv"   -> (getRootTrafo root, config.views, config.projs) |||> AVal.map3 (fun a b c -> a * b.[1] * c.[1]) |> AVal.map (fun t -> t.Inverse) :> IAdaptiveValue |> Some
        ...
        let data = 
            AVal.custom (fun t ->
                ...
                if HashMap.isEmpty ids then
                    ...
                    ids |> HashMap.iter (fun root id ->
                        match getRootUniform "LeftModelViewProjTrafos" root with
                        | Some v ->
                            match v.GetValueUntyped(t) with
                            | :? Trafo3d as b ->
        ...
        let data = 
            AVal.custom (fun t ->
                ...
                if HashMap.isEmpty ids then
                    ...
                    let data : M44f[] = Array.zeroCreate (1 + maxId)
                    ids |> HashMap.iter (fun root id ->
                        match getRootUniform "RightModelViewProjTrafos" root with
                        | Some v ->
                            match v.GetValueUntyped(t) with
                            | :? Trafo3d as b ->
                                data.[id] <- M44f.op_Explicit b.Forward
    ...
    let inner =
        { new DrawPool(ctx, config.alphaToCoverage, true, pRenderBounds, activeBuffer.Pointer, leftModelViewProjBuffer.Pointer, state, config.pass) with
    ...
    let cameraPrediction, puller, thread =
        ...
        let cameraPrediction =
            startThread (fun () ->
                ...
                while not shutdown.IsCancellationRequested do
                    timer.Wait()
                    let view = config.views.GetValue()
        ...
        let thread = 
            startThread (fun () ->
                ...
                let subs =
                    Dict.ofList [
                        config.views :> IAdaptiveObject, config.views.AddMarkingCallback (MVar.put notConverged)
                        config.projs :> IAdaptiveObject, config.projs.AddMarkingCallback (MVar.put notConverged)
                ...
                try 
                    ...
                    while not shutdown.IsCancellationRequested do
                        ...
                        let views = config.views.GetValue AdaptiveToken.Top
                        let projs = config.projs.GetValue AdaptiveToken.Top
                        ...
                        if maxSplits > 0 then
                            ...
                            let maxQ =
                                ...
                                else fst (TreeHelpers.getMaxQuality budget splitfactor (Seq.map fst roots) modelView projs.[0])
                            ...
                            let queue = List()
                            for (k,v) in roots do
                                v.BuildQueue(state, collapseIfNotSplit, k, maxQ, splitfactor, modelView, projs.[0], queue)
            
                            let q, fin = TaskTree<_>.ProcessQueue(state, queue, maxQ, splitfactor, modelView, projs.[0], maxSplits)
                            lastQ <- q
                            let dt = time() - start
    ...
    override x.GetResources() = 
        Seq.concat [ 
            ...
            Seq.singleton (leftModelViewProjBuffer :> IResource)
